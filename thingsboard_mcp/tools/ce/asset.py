...
def delete_asset_profile(asset_profile_id: str) -> str:
    """
...
Deletes the asset profile. Referencing non-existing asset profile Id will cause an error. Can't delete the asset profile if it is referenced by existing assets.  Available for users with 'TENANT_ADMIN' authority.  # noqa: E501
    ...
    try:
        client = get_client()
        result = client.delete_asset_profile(asset_profile_id=asset_profile_id)
        return format_response(result)
    except ApiException as e:
        if e.status == 403:
...
def get_asset_profile_info_by_id(asset_profile_id: str) -> str:
    """
...
Fetch the Asset Profile Info object based on the provided Asset Profile Id. Asset Profile Info is a lightweight object that includes main information about Asset Profile.   Available for users with 'TENANT_ADMIN' or 'CUSTOMER_USER' authority.  # noqa: E501
    ...
    try:
        client = get_client()
        result = client.get_asset_profile_info_by_id(asset_profile_id=asset_profile_id)
        return format_response(result)
    except ApiException as e:
        if e.status == 403:
...
def get_customer_asset_infos(customer_id_json: str, page_size: int, page: int, type: Optional[str] = None, text_search: Optional[str] = None, sort_property: Optional[str] = None, sort_order: Optional[str] = None, asset_profile_id_json: str) -> str:
    """
...
def get_tenant_asset_infos(page_size: int, page: int, type: Optional[str] = None, text_search: Optional[str] = None, sort_property: Optional[str] = None, sort_order: Optional[str] = None, asset_profile_id_json: str) -> str:
    """
...
def save_asset(body_json: str) -> str:
    """
...
def set_default_asset_profile(asset_profile_id: str) -> str:
    """
...
Marks asset profile as default within a tenant scope.  Available for users with 'TENANT_ADMIN' authority.  # noqa: E501
    ...
    try:
        client = get_client()
        result = client.set_default_asset_profile(asset_profile_id=asset_profile_id)
        return format_response(result)
    except ApiException as e:
        if e.status == 403:
