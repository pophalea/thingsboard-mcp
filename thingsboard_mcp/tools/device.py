...
:return: Device
    ...
    try:
        ...
        if e.status == 403:
            return "PERMISSION DENIED: Your role cannot perform 'assign_device_to_tenant'."
        if e.status == 401:
...
:return: DeferredResultResponseEntity
    ...
    try:
        ...
        if e.status == 403:
            return "PERMISSION DENIED: Your role cannot perform 'claim_device'."
        if e.status == 401:
            return "AUTH ERROR: Session expired."
        return f"API Error {e.status}: {e.reason}\n{e.body}"
    except Exception as e:
...
:return: int
    ...
    try:
        ...
        if e.status == 403:
            return "PERMISSION DENIED: Your role cannot perform 'count_by_device_group_and_empty_ota_package'."
        if e.status == 401:
            return "AUTH ERROR: Session expired."
        return f"API Error {e.status}: {e.reason}\n{e.body}"
    except Exception as e:
...
:return: int
    ...
    try:
        ...
        if e.status == 403:
            return "PERMISSION DENIED: Your role cannot perform 'count_by_device_profile_and_empty_ota_package'."
        if e.status == 401:
            return "AUTH ERROR: Session expired."
        return f"API Error {e.status}: {e.reason}\n{e.body}"
    except Exception as e:
...
:return: None
    ...
    try:
        ...
        if e.status == 403:
            return "PERMISSION DENIED: Your role cannot perform 'delete_device'."
        if e.status == 401:
            return "AUTH ERROR: Session expired."
        return f"API Error {e.status}: {e.reason}\n{e.body}"
    except Exception as e:
...
:return: DeferredResultResponseEntity
    ...
    try:
        ...
        if e.status == 403:
            return "PERMISSION DENIED: Your role cannot perform 'delete_device_attributes'."
        if e.status == 401:
            return "AUTH ERROR: Session expired."
        return f"API Error {e.status}: {e.reason}\n{e.body}"
    except Exception as e:
...
:return: None
    ...
    try:
        ...
        if e.status == 403:
            return "PERMISSION DENIED: Your role cannot perform 'delete_device_group_ota_package'."
        if e.status == 401:
            return "AUTH ERROR: Session expired."
        return f"API Error {e.status}: {e.reason}\n{e.body}"
    except Exception as e:
...
:return: None
    ...
    try:
        ...
        if e.status == 403:
            return "PERMISSION DENIED: Your role cannot perform 'delete_device_profile'."
        if e.status == 401:
            return "AUTH ERROR: Session expired."
        return f"API Error {e.status}: {e.reason}\n{e.body}"
    except Exception as e:
...
def get_all_device_infos(page_size: int, page: int, text_search: Optional[str] = None, sort_property: Optional[str] = None, sort_order: Optional[str] = None, include_customers: Optional[str] = None, device_profile_id: Optional[str] = None) -> str:
    """
...
:return: PageDataDeviceInfo
    ...
    try:
        client = get_client()
        result = client.get_all_device_infos(page_size=page_size, page=page, text_search=text_search, sort_property=sort_property, sort_order=sort_order, include_customers=include_customers, device_profile_id=device_profile_id)
        return format_response(result)
    except ApiException as e:
        if e.status == 403:
            return "PERMISSION DENIED: Your role cannot perform 'get_all_device_infos'."
        if e.status == 401:
            return "AUTH ERROR: Session expired."
        return f"API Error {e.status}: {e.reason}\n{e.body}"
    except Exception as e:
...
def get_customer_device_infos(customer_id_json: str, page_size: int, page: int, type: Optional[str] = None, device_profile_id: Optional[str] = None, text_search: Optional[str] = None, sort_property: Optional[str] = None, sort_order: Optional[str] = None, active: Optional[str] = None, include_customers: Optional[str] = None) -> str:
    """
...
:return: PageDataDeviceInfo
    ...
    try:
        client = get_client()
        result = client.get_customer_device_infos(customer_id=deserialize_param(customer_id_json, 'CustomerId'), page_size=page_size, page=page, type=type, device_profile_id=device_profile_id, text_search=text_search, sort_property=sort_property, sort_order=sort_order, active=active, include_customers=include_customers)
        return format_response(result)
    except ApiException as e:
        if e.status == 403:
            return "PERMISSION DENIED: Your role cannot perform 'get_customer_device_infos'."
        if e.status == 401:
            return "AUTH ERROR: Session expired."
        return f"API Error {e.status}: {e.reason}\n{e.body}"
    except Exception as e:
...
:return: PageDataDevice
    ...
    try:
        ...
        if e.status == 403:
            return "PERMISSION DENIED: Your role cannot perform 'get_customer_devices'."
        if e.status == 401:
            return "AUTH ERROR: Session expired."
        return f"API Error {e.status}: {e.reason}\n{e.body}"
    except Exception as e:
...
:return: DeviceProfileInfo
    ...
    try:
        ...
        if e.status == 403:
            return "PERMISSION DENIED: Your role cannot perform 'get_default_device_profile_info'."
        if e.status == 401:
            return "AUTH ERROR: Session expired."
        return f"API Error {e.status}: {e.reason}\n{e.body}"
    except Exception as e:
...
:return: DeferredResultResponseEntity
    ...
    try:
        ...
        if e.status == 403:
            return "PERMISSION DENIED: Your role cannot perform 'get_device_attributes'."
        if e.status == 401:
            return "AUTH ERROR: Session expired."
        return f"API Error {e.status}: {e.reason}\n{e.body}"
    except Exception as e:
...
:return: Device
    ...
    try:
        ...
        if e.status == 403:
            return "PERMISSION DENIED: Your role cannot perform 'get_device_by_id'."
        if e.status == 401:
            return "AUTH ERROR: Session expired."
        return f"API Error {e.status}: {e.reason}\n{e.body}"
    except Exception as e:
...
:return: DeviceCredentials
    ...
    try:
        ...
        if e.status == 403:
            return "PERMISSION DENIED: Your role cannot perform 'get_device_credentials_by_device_id'."
        if e.status == 401:
            return "AUTH ERROR: Session expired."
        return f"API Error {e.status}: {e.reason}\n{e.body}"
    except Exception as e:
...
:return: DeviceInfo
    ...
    try:
        ...
        if e.status == 403:
            return "PERMISSION DENIED: Your role cannot perform 'get_device_info_by_id'."
        if e.status == 401:
            return "AUTH ERROR: Session expired."
        return f"API Error {e.status}: {e.reason}\n{e.body}"
    except Exception as e:
...
def get_device_profile_by_id(device_profile_id_json: str, inline_images: Optional[str] = None) -> str:
    """
...
:return: DeviceProfile
    ...
    try:
        ...
        if e.status == 403:
            return "PERMISSION DENIED: Your role cannot perform 'get_device_profile_by_id'."
        if e.status == 401:
            return "AUTH ERROR: Session expired."
        return f"API Error {e.status}: {e.reason}\n{e.body}"
    except Exception as e:
...
:return: DeviceProfileInfo
    ...
    try:
        ...
        if e.status == 403:
            return "PERMISSION DENIED: Your role cannot perform 'get_device_profile_info_by_id'."
        if e.status == 401:
            return "AUTH ERROR: Session expired."
        return f"API Error {e.status}: {e.reason}\n{e.body}"
    except Exception as e:
...
:return: PageDataDeviceProfileInfo
    ...
    try:
        ...
        if e.status == 403:
            return "PERMISSION DENIED: Your role cannot perform 'get_device_profile_infos'."
        if e.status == 401:
            return "AUTH ERROR: Session expired."
        return f"API Error {e.status}: {e.reason}\n{e.body}"
    except Exception as e:
...
def get_device_profile_names(active_only: str) -> str:
    """
...
:return: list[EntityInfo]
    ...
    try:
        ...
        if e.status == 403:
            return "PERMISSION DENIED: Your role cannot perform 'get_device_profile_names'."
        if e.status == 401:
            return "AUTH ERROR: Session expired."
        return f"API Error {e.status}: {e.reason}\n{e.body}"
    except Exception as e:
...
:return: PageDataDeviceProfile
    ...
    try:
        ...
        if e.status == 403:
            return "PERMISSION DENIED: Your role cannot perform 'get_device_profiles'."
        if e.status == 401:
            return "AUTH ERROR: Session expired."
        return f"API Error {e.status}: {e.reason}\n{e.body}"
    except Exception as e:
...
:return: list[DeviceProfileInfo]
    ...
    try:
        ...
        if e.status == 403:
            return "PERMISSION DENIED: Your role cannot perform 'get_device_profiles_by_ids'."
        if e.status == 401:
            return "AUTH ERROR: Session expired."
        return f"API Error {e.status}: {e.reason}\n{e.body}"
    except Exception as e:
...
:return: JsonNode
    ...
    try:
        ...
        if e.status == 403:
            return "PERMISSION DENIED: Your role cannot perform 'get_device_publish_telemetry_commands'."
        if e.status == 401:
            return "AUTH ERROR: Session expired."
        return f"API Error {e.status}: {e.reason}\n{e.body}"
    except Exception as e:
...
:return: list[EntitySubtype]
    ...
    try:
        ...
        if e.status == 403:
            return "PERMISSION DENIED: Your role cannot perform 'get_device_types'."
        if e.status == 401:
            return "AUTH ERROR: Session expired."
        return f"API Error {e.status}: {e.reason}\n{e.body}"
    except Exception as e:
...
:return: PageDataDevice
    ...
    try:
        ...
        if e.status == 403:
            return "PERMISSION DENIED: Your role cannot perform 'get_devices_by_entity_group_id'."
        if e.status == 401:
            return "AUTH ERROR: Session expired."
        return f"API Error {e.status}: {e.reason}\n{e.body}"
    except Exception as e:
...
:return: list[Device]
    ...
    try:
        ...
        if e.status == 403:
            return "PERMISSION DENIED: Your role cannot perform 'get_devices_by_ids'."
        if e.status == 401:
            return "AUTH ERROR: Session expired."
        return f"API Error {e.status}: {e.reason}\n{e.body}"
    except Exception as e:
...
:return: DeferredResultResponseEntity
    ...
    try:
        ...
        if e.status == 403:
            return "PERMISSION DENIED: Your role cannot perform 'get_persisted_rpc_by_device'."
        if e.status == 401:
            return "AUTH ERROR: Session expired."
        return f"API Error {e.status}: {e.reason}\n{e.body}"
    except Exception as e:
...
:return: Device
    ...
    try:
        ...
        if e.status == 403:
            return "PERMISSION DENIED: Your role cannot perform 'get_tenant_device'."
        if e.status == 401:
            return "AUTH ERROR: Session expired."
        return f"API Error {e.status}: {e.reason}\n{e.body}"
    except Exception as e:
...
:return: PageDataDevice
    ...
    try:
        ...
        if e.status == 403:
            return "PERMISSION DENIED: Your role cannot perform 'get_tenant_devices'."
        if e.status == 401:
            return "AUTH ERROR: Session expired."
        return f"API Error {e.status}: {e.reason}\n{e.body}"
    except Exception as e:
...
:return: PageDataDevice
    ...
    try:
        ...
        if e.status == 403:
            return "PERMISSION DENIED: Your role cannot perform 'get_user_devices'."
        if e.status == 401:
            return "AUTH ERROR: Session expired."
        return f"API Error {e.status}: {e.reason}\n{e.body}"
    except Exception as e:
...
:return: DeferredResultResponseEntity
    ...
    try:
        ...
        if e.status == 403:
            return "PERMISSION DENIED: Your role cannot perform 'handle_one_way_device_rpc_request'."
        if e.status == 401:
            return "AUTH ERROR: Session expired."
        return f"API Error {e.status}: {e.reason}\n{e.body}"
    except Exception as e:
...
def handle_one_way_device_rpc_request_v1(device_id_json: str, body: Optional[str] = None) -> str:
    ...
    try:
        ...
        if e.status == 403:
            return "PERMISSION DENIED: Your role cannot perform 'handle_one_way_device_rpc_request_v1'."
        if e.status == 401:
            return "AUTH ERROR: Session expired."
        return f"API Error {e.status}: {e.reason}\n{e.body}"
    except Exception as e:
...
def handle_two_way_device_rpc_request(device_id_json: str, body: Optional[str] = None) -> str:
    """
...
:return: DeferredResultResponseEntity
    ...
    try:
        client = get_client()
        result = client.handle_two_way_device_rpc_request(device_id=deserialize_param(device_id_json, 'DeviceId'), body=body)
        return format_response(result)
    except ApiException as e:
        if e.status == 403:
            return "PERMISSION DENIED: Your role cannot perform 'handle_two_way_device_rpc_request'."
        if e.status == 401:
            return "AUTH ERROR: Session expired."
        return f"API Error {e.status}: {e.reason}\n{e.body}"
    except Exception as e:
...
def handle_two_way_device_rpc_request_v1(device_id_json: str, body: Optional[str] = None) -> str:
    ...
    try:
        ...
        if e.status == 403:
            return "PERMISSION DENIED: Your role cannot perform 'handle_two_way_device_rpc_request_v1'."
        if e.status == 401:
            return "AUTH ERROR: Session expired."
        return f"API Error {e.status}: {e.reason}\n{e.body}"
    except Exception as e:
...
:return: DeferredResultResponseEntity
    ...
    try:
        ...
        if e.status == 403:
            return "PERMISSION DENIED: Your role cannot perform 'post_device_attributes'."
        if e.status == 401:
            return "AUTH ERROR: Session expired."
        return f"API Error {e.status}: {e.reason}\n{e.body}"
    except Exception as e:
...
def process_devices_bulk_import(body: Optional[str] = None) -> str:
    """
...
:return: BulkImportResultDevice
    ...
    try:
        client = get_client()
        result = client.process_devices_bulk_import(body=body)
        return format_response(result)
    except ApiException as e:
        if e.status == 403:
            return "PERMISSION DENIED: Your role cannot perform 'process_devices_bulk_import'."
        if e.status == 401:
            return "AUTH ERROR: Session expired."
        return f"API Error {e.status}: {e.reason}\n{e.body}"
    except Exception as e:
...
:return: DeferredResultResponseEntity
    ...
    try:
        ...
        if e.status == 403:
            return "PERMISSION DENIED: Your role cannot perform 'provision_device'."
        if e.status == 401:
            return "AUTH ERROR: Session expired."
        return f"API Error {e.status}: {e.reason}\n{e.body}"
    except Exception as e:
...
:return: DeferredResultResponseEntity
    ...
    try:
        ...
        if e.status == 403:
            return "PERMISSION DENIED: Your role cannot perform 're_claim_device'."
        if e.status == 401:
            return "AUTH ERROR: Session expired."
        return f"API Error {e.status}: {e.reason}\n{e.body}"
    except Exception as e:
...
def save_device(body: str, access_token: Optional[str] = None) -> str:
    """
...
:return: Device
    ...
    try:
        client = get_client()
        result = client.save_device(body=body, access_token=access_token)
        return format_response(result)
    except ApiException as e:
        if e.status == 403:
            return "PERMISSION DENIED: Your role cannot perform 'save_device'."
        if e.status == 401:
            return "AUTH ERROR: Session expired."
        return f"API Error {e.status}: {e.reason}\n{e.body}"
    except Exception as e:
...
:return: DeferredResultResponseEntity
    ...
    try:
        ...
        if e.status == 403:
            return "PERMISSION DENIED: Your role cannot perform 'save_device_attributes'."
        if e.status == 401:
            return "AUTH ERROR: Session expired."
        return f"API Error {e.status}: {e.reason}\n{e.body}"
    except Exception as e:
...
def save_device_group_ota_package(body: Optional[str] = None) -> str:
    """
...
:param async_req bool
:param DeviceGroupOtaPackage body:
:return: DeviceGroupOtaPackage
    ...
    try:
        client = get_client()
        result = client.save_device_group_ota_package(body=body)
        return format_response(result)
    except ApiException as e:
        if e.status == 403:
            return "PERMISSION DENIED: Your role cannot perform 'save_device_group_ota_package'."
        if e.status == 401:
            return "AUTH ERROR: Session expired."
        return f"API Error {e.status}: {e.reason}\n{e.body}"
    except Exception as e:
...
def save_device_profile(body: Optional[str] = None) -> str:
    """
...
:return: DeviceProfile
    ...
    try:
        client = get_client()
        result = client.save_device_profile(body=body)
        return format_response(result)
    except ApiException as e:
        if e.status == 403:
            return "PERMISSION DENIED: Your role cannot perform 'save_device_profile'."
        if e.status == 401:
            return "AUTH ERROR: Session expired."
        return f"API Error {e.status}: {e.reason}\n{e.body}"
    except Exception as e:
...
def save_device_with_credentials(body: Optional[str] = None, entity_group_id: Optional[str] = None, entity_group_ids: Optional[str] = None) -> str:
    """
    Save device with credentials (Deprecated)  # noqa: E501
...
:return: Device
    ...
    try:
        client = get_client()
        result = client.save_device_with_credentials(body=body, entity_group_id=entity_group_id, entity_group_ids=entity_group_ids)
        return format_response(result)
    except ApiException as e:
        if e.status == 403:
            return "PERMISSION DENIED: Your role cannot perform 'save_device_with_credentials'."
        if e.status == 401:
            return "AUTH ERROR: Session expired."
        return f"API Error {e.status}: {e.reason}\n{e.body}"
    except Exception as e:
...
:return: DeviceProfile
    ...
    try:
        ...
        if e.status == 403:
            return "PERMISSION DENIED: Your role cannot perform 'set_default_device_profile'."
        if e.status == 401:
            return "AUTH ERROR: Session expired."
        return f"API Error {e.status}: {e.reason}\n{e.body}"
    except Exception as e:
...
def update_device_credentials(body: Optional[str] = None) -> str:
    """
...
:return: DeviceCredentials
    ...
    try:
        client = get_client()
        result = client.update_device_credentials(body=body)
        return format_response(result)
    except ApiException as e:
        if e.status == 403:
            return "PERMISSION DENIED: Your role cannot perform 'update_device_credentials'."
        if e.status == 401:
            return "AUTH ERROR: Session expired."
        return f"API Error {e.status}: {e.reason}\n{e.body}"
    except Exception as e:
