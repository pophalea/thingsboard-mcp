...
:return: None
    ...
    try:
        ...
        if e.status == 403:
            return "PERMISSION DENIED: Your role cannot perform 'delete_ota_package'."
        if e.status == 401:
...
:return: Resource
    ...
    try:
        ...
        if e.status == 403:
            return "PERMISSION DENIED: Your role cannot perform 'download_ota_package'."
        if e.status == 401:
            return "AUTH ERROR: Session expired."
        return f"API Error {e.status}: {e.reason}\n{e.body}"
    except Exception as e:
...
:return: PageDataOtaPackageInfo
    ...
    try:
        ...
        if e.status == 403:
            return "PERMISSION DENIED: Your role cannot perform 'get_group_ota_packages'."
        if e.status == 401:
            return "AUTH ERROR: Session expired."
        return f"API Error {e.status}: {e.reason}\n{e.body}"
    except Exception as e:
...
:return: OtaPackage
    ...
    try:
        ...
        if e.status == 403:
            return "PERMISSION DENIED: Your role cannot perform 'get_ota_package_by_id'."
        if e.status == 401:
            return "AUTH ERROR: Session expired."
        return f"API Error {e.status}: {e.reason}\n{e.body}"
    except Exception as e:
...
:return: OtaPackageInfo
    ...
    try:
        ...
        if e.status == 403:
            return "PERMISSION DENIED: Your role cannot perform 'get_ota_package_info_by_id'."
        if e.status == 401:
            return "AUTH ERROR: Session expired."
        return f"API Error {e.status}: {e.reason}\n{e.body}"
    except Exception as e:
...
:return: PageDataOtaPackageInfo
    ...
    try:
        ...
        if e.status == 403:
            return "PERMISSION DENIED: Your role cannot perform 'get_ota_packages'."
        if e.status == 401:
            return "AUTH ERROR: Session expired."
        return f"API Error {e.status}: {e.reason}\n{e.body}"
    except Exception as e:
...
def get_ota_packages_v1(device_profile_id_json: str, type: str, page_size: int, page: int, text_search: Optional[str] = None, sort_property: Optional[str] = None, sort_order: Optional[str] = None) -> str:
    ...
    try:
        ...
        if e.status == 403:
            return "PERMISSION DENIED: Your role cannot perform 'get_ota_packages_v1'."
        if e.status == 401:
            return "AUTH ERROR: Session expired."
        return f"API Error {e.status}: {e.reason}\n{e.body}"
    except Exception as e:
...
def save_ota_package_data(ota_package_id_json: str, checksum: Optional[str] = None, checksum_algorithm: Optional[str] = None, file: Optional[str] = None) -> str:
    """
...
:return: OtaPackageInfo
    ...
    try:
        client = get_client()
        result = client.save_ota_package_data(ota_package_id=deserialize_param(ota_package_id_json, 'OtaPackageId'), checksum=checksum, checksum_algorithm=checksum_algorithm, file=file)
        return format_response(result)
    except ApiException as e:
        if e.status == 403:
            return "PERMISSION DENIED: Your role cannot perform 'save_ota_package_data'."
        if e.status == 401:
            return "AUTH ERROR: Session expired."
        return f"API Error {e.status}: {e.reason}\n{e.body}"
    except Exception as e:
...
def save_ota_package_info(body: Optional[str] = None) -> str:
    """
...
:return: OtaPackageInfo
    ...
    try:
        client = get_client()
        result = client.save_ota_package_info(body=body)
        return format_response(result)
    except ApiException as e:
        if e.status == 403:
            return "PERMISSION DENIED: Your role cannot perform 'save_ota_package_info'."
        if e.status == 401:
            return "AUTH ERROR: Session expired."
        return f"API Error {e.status}: {e.reason}\n{e.body}"
    except Exception as e:
